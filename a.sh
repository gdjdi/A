...
# 创建快捷运行
if ! command -v a >/dev/null 2>&1; then [[ $EUID -ne 0 ]] && echo "无root权" && exit 1; curl -sL "https://juh.cc/a.sh" -o "/usr/local/bin/a" && chmod +x "/usr/local/bin/a" && echo "可以 a 使用" || { echo "安装失败"; exit 1; }; fi
